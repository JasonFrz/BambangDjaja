...
const TransformerSelection = () => {
  ...
  // WhatsApp state
  const [waSending, setWaSending] = useState(null); // trafo id being sent
  const [waStatus, setWaStatus] = useState({}); // { [trafoId]: { type: 'success'|'error', msg: '...' } }
  ...
  const handleMonitor = (trafo) => {
    ...
    setTimeout(() => {
      navigate('/dashboard');
    }, 2500); // Wait 2.5s for the animation to play
  };
