...
import {
  ...
  RefreshCw, Server, Search, Users, UserPlus, Edit2, X, Check,
  Eye, EyeOff, LayoutDashboard, Download, Menu, LogOut, CheckCircle2, XCircle, MessageCircle, ArrowLeft, ChevronDown, Settings, Sun, Moon, Building2
} from 'lucide-react';
    ...
    <div className={`relative ${className}`}>
      ...
      {isOpen && (
        <>
          ...
          <div className="absolute right-0 left-0 w-full top-full mt-2 bg-white dark:bg-[#1a1a24] border border-gray-200 dark:border-white/10 rounded-xl shadow-[0_8px_30px_rgb(0,0,0,0.5)] z-50 py-1 overflow-hidden">
            {options.map(opt => (
              <button
                key={opt.value}
...
const SidebarItem = ({ icon: Icon, label, id, isCustomIcon, activeTab, setActiveTab, isSidebarCollapsed }) => (
  <button
    onClick={() => setActiveTab(id)}
    className={`w-full flex items-center gap-3 px-4 py-3 rounded-xl transition-all font-semibold text-sm ${activeTab === id
  ...
  >
    {isCustomIcon ? (
      <img src="/wa.png" alt="WA" className={`object-contain shrink-0 ${isSidebarCollapsed ? 'w-6 h-6' : 'w-5 h-5'}`} style={{ filter: activeTab === id ? 'brightness(1)' : 'grayscale(100%) opacity(0.7)' }} />
    ) : (
      <Icon size={isSidebarCollapsed ? 24 : 20} className="shrink-0" />
    )}
...
const BottomNavItem = ({ icon: Icon, label, id, isCustomIcon, activeTab, setActiveTab }) => (
  <button
    onClick={() => setActiveTab(id)}
    className={`flex flex-col items-center justify-center p-2 rounded-lg transition-colors min-w-[72px] shrink-0 ${activeTab === id ? 'text-blue-400' : 'text-gray-500 hover:text-gray-300'
      }`}
  >
    {isCustomIcon ? (
...
const AdminDashboard = () => {
  ...
  const handleDeleteAdmin = async (id) => {
    const isConfirmed = await confirm('Are you sure you want to delete this admin?');
  ...
  const handleSaveInlineAppUser = async (id) => {
    try {
      await axiosInstance.put(`/api/admin/app-users/${id}`, inlineAppUserForm);
      setInlineEditingAppUserId(null);
      fetchAllUsers();
    } catch (err) {
      alert(err.response?.data?.error || 'Failed to save app user');
    }
  };

  const handleDeleteAppUser = async (id) => {
    const isConfirmed = await confirm('Are you sure you want to delete this app user?');
    if (!isConfirmed) return;
    try {
  ...
  const handleSaveCompany = async (e) => {
    ...
    try {
      if (editingCompany) {
        await axiosInstance.put(`/api/admin/companies/${editingCompany.id}`, companyForm);
      }
      setShowCompanyModal(false);
      fetchCompanies();
    } catch (err) {
      setCompanyFormError(err.response?.data?.error || 'Failed to save company');
    } finally {
  ...
  const handleDeleteCompany = async (id) => {
    const isConfirmed = await confirm('Are you sure you want to delete this company? This action is permanent and will drop the database and all its associated users.');
    if (!isConfirmed) return;
    try {
  ...
  return (
    <div className={`flex h-screen overflow-hidden ${theme === 'dark' ? 'dark bg-[#f4f7fe] dark:bg-[#101014]' : 'bg-[#f4f7fe]'} text-[#172b4d] dark:text-white font-sans transition-colors duration-300`}>
      ...
      <aside className={`hidden md:flex flex-col inset-y-0 left-0 z-50 transition-all duration-300 bg-white dark:bg-[#0b0c10] border-r border-gray-200 dark:border-white/5 ${isSidebarCollapsed ? 'w-20' : 'w-72'}`}>
        ...
        <div className={`px-6 py-5 ${isSidebarCollapsed ? 'flex justify-center px-2' : ''}`}>
          {!isSidebarCollapsed ? (
        ...
        <nav className="flex-1 px-4 space-y-6 overflow-y-auto custom-scrollbar pb-6">
          <div>
            <SidebarItem icon={LayoutDashboard} label="Overview" id="overview" activeTab={activeTab} setActiveTab={setActiveTab} isSidebarCollapsed={isSidebarCollapsed} />
          </div>

          <div>
            {!isSidebarCollapsed && <p className="px-4 text-[10px] font-bold text-gray-500 uppercase tracking-wider mb-2">Database</p>}
            <SidebarItem icon={Database} label="Databases" id="databases" activeTab={activeTab} setActiveTab={setActiveTab} isSidebarCollapsed={isSidebarCollapsed} />
          </div>

          <div>
            {!isSidebarCollapsed && <p className="px-4 text-[10px] font-bold text-gray-500 uppercase tracking-wider mb-2">Access Management</p>}
            <SidebarItem icon={Building2} label="Companies" id="companies" activeTab={activeTab} setActiveTab={setActiveTab} isSidebarCollapsed={isSidebarCollapsed} />
            <SidebarItem icon={Users} label="Admin Users" id="admins" activeTab={activeTab} setActiveTab={setActiveTab} isSidebarCollapsed={isSidebarCollapsed} />
            <SidebarItem icon={UserPlus} label="App Users" id="users" activeTab={activeTab} setActiveTab={setActiveTab} isSidebarCollapsed={isSidebarCollapsed} />
          </div>

          <div>
            {!isSidebarCollapsed && <p className="px-4 text-[10px] font-bold text-gray-500 uppercase tracking-wider mb-2">Services</p>}
            <SidebarItem icon={null} label="Notifications" id="whatsapp" isCustomIcon={true} activeTab={activeTab} setActiveTab={setActiveTab} isSidebarCollapsed={isSidebarCollapsed} />
          </div>

          <div>
            {!isSidebarCollapsed && <p className="px-4 text-[10px] font-bold text-gray-500 uppercase tracking-wider mb-2">System</p>}
            <SidebarItem icon={Settings} label="Settings" id="settings" activeTab={activeTab} setActiveTab={setActiveTab} isSidebarCollapsed={isSidebarCollapsed} />
          </div>
        </nav>
        ...
        <div className="p-4 border-t border-gray-200 dark:border-white/5 shrink-0">
          ...
          <div className="flex justify-center border-t border-[#dfe1e6] dark:border-white/10 pt-4 mt-2">
            <button 
      ...
      <main className="flex-1 flex flex-col min-w-0 bg-white dark:bg-[#0b0c10] pb-[72px] md:pb-0">
        ...
        <header className="hidden md:flex items-center justify-between p-6 border-b border-gray-200 dark:border-white/5 bg-white dark:bg-[#0b0c10]">
          ...
          <div className="flex items-center gap-6">
            ...
            <button onClick={handleLogout} className="px-4 py-2 rounded-lg bg-white border border-red-500 text-red-600 hover:bg-red-50 dark:bg-red-900/20 dark:border-red-500/20 dark:text-red-400 dark:hover:bg-red-900/30 dark:hover:text-red-300 flex items-center gap-2 transition-colors">
              <LogOut size={16} />
        ...
        <div className="md:hidden flex items-center justify-between p-4 bg-white dark:bg-[#0b0c10] border-b border-gray-200 dark:border-white/5 shrink-0 z-20 sticky top-0">
          ...
          <button onClick={handleLogout} className="p-2 border border-red-500 bg-white text-red-600 hover:bg-red-50 dark:border-transparent dark:bg-red-900/20 dark:text-red-400 dark:hover:bg-red-900/30 dark:hover:text-red-300 transition-colors rounded-lg" title="Logout">
            <LogOut size={18} />
          </button>
        </div>
        ...
        <div className="flex-1 overflow-auto custom-scrollbar relative">
            ...
            <div className="p-4 md:p-8 w-full mx-auto space-y-6 md:space-y-8 animate-[fadeIn_0.3s_ease-out]">
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4 md:gap-6 mb-8">
                {[
                  ...
                  <div key={i} className={`bg-[#f4f7fe] dark:bg-[#101014] border-t-2 ${stat.color === 'blue' ? 'border-t-blue-500' :
                    ...
                    <div className={`absolute -top-10 -right-10 w-32 h-32 ${stat.color === 'blue' ? 'bg-blue-500/5' :
                    ...
                    <div className="flex items-center gap-4 mb-4 relative z-10">
                      <div className={`w-12 h-12 rounded-xl flex items-center justify-center ${stat.color === 'blue' ? 'bg-white text-blue-600 border-blue-500 dark:bg-blue-900/20 dark:text-blue-400 dark:border-blue-500/20' :
                          stat.color === 'cyan' ? 'bg-white text-cyan-600 border-cyan-500 dark:bg-cyan-900/20 dark:text-cyan-400 dark:border-cyan-500/20' :
                            stat.color === 'purple' ? 'bg-white text-purple-600 border-purple-500 dark:bg-purple-900/20 dark:text-purple-400 dark:border-purple-500/20' :
                              'bg-white text-green-600 border-green-500 dark:bg-green-900/20 dark:text-green-400 dark:border-green-500/20'
                        } border`}>
                        <stat.icon size={20} />
                      </div>
                      <div>
              ...
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-8">
                  ...
                  <div className="space-y-4 max-h-64 overflow-y-auto custom-scrollbar pr-2 relative z-10">
                    {isLoadingDbs ? <EnergyLoader size="small" /> : databases.map(db => (
                      <div key={db} className="bg-white dark:bg-[#151521] border border-gray-200 dark:border-white/5 rounded-xl p-4 flex items-center justify-between transition-colors hover:bg-gray-100 dark:hover:bg-gray-800">
                        <div className="flex items-center gap-4">
                          <div className="w-10 h-10 bg-white border border-blue-500 text-blue-600 dark:bg-blue-900/20 dark:border-transparent dark:text-blue-400 rounded-xl flex items-center justify-center">
                            <Database size={20} />
                          </div>
                          <div>
                        ...
                        <span className="px-3 py-1 bg-white text-green-600 border border-green-500 dark:bg-green-900/20 dark:border-green-500/20 dark:text-green-400 text-[10px] font-bold rounded-lg uppercase tracking-wider">online</span>
                      </div>
                    ))}
                  </div>
                </div>
                ...
                <div className="bg-[#f4f7fe] dark:bg-[#101014] border border-gray-200 dark:border-white/5 rounded-2xl p-6 relative overflow-hidden group flex flex-col justify-between">
                  ...
                  <button
                    ...
                    className={`w-full py-3 rounded-xl border font-bold transition-all mt-6 relative z-10 text-sm ${waStatus.ready ? 'bg-white border-red-500 text-red-600 hover:bg-red-50 dark:bg-transparent dark:border-red-900/50 dark:text-red-400 dark:hover:bg-red-900/20' : 'bg-white dark:bg-[#151521] border-blue-900/50 text-blue-400 hover:bg-blue-900/20 hover:border-blue-500/50 shadow-lg shadow-blue-900/10'}`}
                  >
              ...
              <div className="bg-[#f4f7fe] dark:bg-[#101014] border border-gray-200 dark:border-white/5 rounded-2xl overflow-hidden flex flex-col max-h-[70vh]">
                <div className="p-6 border-b border-gray-200 dark:border-white/5 flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
                  ...
                  <div className="flex flex-wrap gap-3 w-full md:w-auto">
                    ...
                    <button onClick={() => setActiveTab('users')} className="px-4 py-2 text-sm font-bold bg-white text-blue-600 border-2 border-blue-600 hover:bg-blue-50 dark:bg-transparent dark:text-blue-400 dark:border-blue-400 dark:hover:bg-blue-900/20 rounded-lg transition-colors flex items-center gap-2 shadow-sm">
                ...
                <div className="flex-1 overflow-auto custom-scrollbar max-h-[300px]">
                  <table className="w-full text-left text-sm whitespace-nowrap relative">
                    <thead className="bg-[#f4f7fe] dark:bg-[#151521] text-gray-500 dark:text-gray-400 sticky top-0 z-10 shadow-sm">
                      <tr>
                    ...
                    <tbody className="divide-y divide-white/5">
                      {allUsers.filter(u => (u.username || '').toLowerCase().includes(searchAllUser.toLowerCase())).map(u => (
                        inlineEditingAppUserId === u.id ? (
                          <tr key={u.id} className="hover:bg-gray-100 dark:hover:bg-gray-800 transition-colors">
                            <td className="py-4 px-6">
                              <input 
                                type="text" 
                                value={inlineAppUserForm.username} 
                                onChange={e => setInlineAppUserForm({...inlineAppUserForm, username: e.target.value})} 
                                className="bg-transparent border-b-2 border-blue-500 outline-none w-full text-sm font-bold text-[#172b4d] dark:text-white" 
                              />
                            </td>
                            <td className="py-4 px-6">
                              <input 
                                type="text" 
                                value={inlineAppUserForm.nomor_telpon} 
                                onChange={e => setInlineAppUserForm({...inlineAppUserForm, nomor_telpon: e.target.value})} 
                                className="bg-transparent border-b-2 border-blue-500 outline-none w-full text-sm text-gray-500 dark:text-gray-400" 
                              />
                            </td>
                            <td className="py-4 px-6">
                              <input 
                                type="text" 
                                value={inlineAppUserForm.email} 
                                onChange={e => setInlineAppUserForm({...inlineAppUserForm, email: e.target.value})} 
                                className="bg-transparent border-b-2 border-blue-500 outline-none w-full text-sm text-gray-500 dark:text-gray-400" 
                              />
                            </td>
                            <td className="py-4 px-6">
                              <select 
                                value={inlineAppUserForm.role} 
                                onChange={e => setInlineAppUserForm({...inlineAppUserForm, role: e.target.value})} 
                                className="bg-white dark:bg-[#111217] border border-blue-500 rounded p-1 text-[10px] font-bold uppercase tracking-wider outline-none text-[#172b4d] dark:text-white"
                              >
                            ...
                            <td className="py-4 px-6 text-center">
                              <div className="flex items-center justify-center gap-2">
                                <button onClick={() => handleSaveInlineAppUser(u.id)} className="p-2 text-green-500 hover:bg-green-100 dark:text-green-400 dark:hover:bg-green-900/20 rounded-lg transition-colors" title="Save"><Check size={16} /></button>
                                <button onClick={() => setInlineEditingAppUserId(null)} className="p-2 text-gray-500 hover:bg-gray-100 dark:text-gray-400 dark:hover:bg-gray-900/20 rounded-lg transition-colors" title="Cancel"><X size={16} /></button>
                              </div>
                            </td>
                          </tr>
                        ) : (
                          <tr key={u.id} className="hover:bg-gray-100 dark:hover:bg-gray-800 transition-colors">
                            <td className="py-4 px-6">
                              <div className="flex items-center gap-3">
                                <div className={`w-8 h-8 rounded-full flex items-center justify-center font-bold text-sm ${u.role === 'admin' ? 'bg-blue-100 text-blue-600 dark:bg-blue-900/30 dark:text-blue-400' :
                                    u.role === 'superuser' ? 'bg-green-100 text-green-600 dark:bg-green-900/30 dark:text-green-400' :
                            ...
                            <td className="py-4 px-6">
                              <span className={`px-2 py-1 rounded text-[10px] font-bold uppercase tracking-wider ${u.role === 'admin' ? 'bg-blue-100 text-blue-600 border border-blue-200 dark:bg-blue-900/20 dark:text-blue-400 dark:border-blue-500/20' :
                                  u.role === 'superuser' ? 'bg-green-100 text-green-600 border border-green-200 dark:bg-green-900/20 dark:text-green-400 dark:border-green-500/20' :
                                    'bg-yellow-100 text-yellow-600 border border-yellow-200 dark:bg-yellow-900/20 dark:text-yellow-400 dark:border-yellow-500/20'
                                }`}>
                                {u.role}
                              </span>
                            </td>
                            <td className="py-4 px-6 text-center">
                              <div className="flex items-center justify-center gap-2">
                                <button onClick={() => { setInlineEditingAppUserId(u.id); setInlineAppUserForm({ username: u.username, password: '', role: u.role, nama_db: u.nama_db || '', nomor_telpon: u.nomor_telpon || '', email: u.email || '' }); }} className="p-2 text-blue-500 hover:bg-blue-100 dark:text-blue-400 dark:hover:bg-blue-900/20 rounded-lg transition-colors" title="Edit"><Edit2 size={16} /></button>
                                <button onClick={() => handleDeleteAppUser(u.id)} className="p-2 text-red-500 hover:bg-red-100 dark:text-red-400 dark:hover:bg-red-900/20 rounded-lg transition-colors" title="Delete"><Trash2 size={16} /></button>
                              </div>
                            </td>
                          </tr>
                        )
          ...
          {activeTab === 'databases' && (
            <>
              ...
              <div className="hidden md:flex flex-col md:flex-row h-full animate-[fadeIn_0.3s_ease-out]">
                ...
                <div className="flex-1 flex flex-col bg-[#f4f7fe] dark:bg-[#1a1a24] overflow-hidden">
                  {selectedTable ? (
                    <>
                      <div className="p-4 md:p-6 border-b border-gray-200 dark:border-white/5 flex flex-col md:flex-row items-start md:items-end justify-between shrink-0 gap-4">
                        <div>
                          <div className="flex items-center gap-3 mb-1">
                            ...
                            <span className="px-2 py-0.5 rounded-full bg-white text-green-600 border border-green-500 dark:bg-green-900/30 dark:border-transparent dark:text-green-400 text-xs font-bold flex items-center gap-1"><div className="w-1.5 h-1.5 rounded-full bg-green-500 dark:bg-green-400" /> online</span>
                          </div>
                  ...
                  <>
                    ...
                    <div className="flex-1 overflow-y-auto space-y-3 pb-20 custom-scrollbar">
                      {databases.filter(d => d.toLowerCase().includes(searchTermDb.toLowerCase())).map(db => (
                        ...
                        >
                          <div className="flex items-center gap-4">
                            <div className="w-12 h-12 bg-gray-100 dark:bg-[#0a0a0f] border border-gray-200 dark:border-white/5 text-blue-400 rounded-xl flex items-center justify-center shadow-inner">
                              <Database size={22} />
                            </div>
                    ...
                    <div className="flex-1 overflow-y-auto space-y-3 pb-20 custom-scrollbar">
                      {isLoadingTables ? <div className="py-10"><EnergyLoader size="medium" /></div> : tables.map(table => (
                        ...
                        >
                          ...
                          <div className="flex items-center gap-3">
                            <span className="px-2.5 py-1 bg-white text-green-600 border border-green-500 dark:bg-green-900/20 dark:border-green-500/20 dark:text-green-400 text-xs font-bold rounded-lg">online</span>
                            <ChevronRight size={18} className="text-gray-500" />
                          </div>
                        </button>
                      ))}
          ...
          {activeTab === 'admins' && (
            <div className="p-4 md:p-8 w-full mx-auto space-y-6 animate-[fadeIn_0.3s_ease-out]">
              <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
                ...
                {!showAdminModal && (
                  <button onClick={() => { setEditingAdmin(null); setAdminForm({ username: '', password: '' }); setShowAdminModal(true); }} className="px-4 py-2 bg-white text-blue-600 border-2 border-blue-600 hover:bg-blue-50 dark:bg-transparent dark:text-blue-400 dark:border-blue-400 dark:hover:bg-blue-900/20 rounded-xl font-bold transition-colors flex items-center gap-2 shadow-sm">
              ...
              {showAdminModal && (
                <div className="bg-[#f4f7fe] dark:bg-[#151521] border border-gray-200 dark:border-white/10 rounded-2xl p-6">
              ...
              <div className="bg-[#f4f7fe] dark:bg-[#151521] border border-gray-200 dark:border-white/10 rounded-2xl p-4 md:p-6 flex flex-col max-h-[70vh]">
                <div className="flex-1 overflow-auto custom-scrollbar">
                  <table className="w-full text-left text-sm relative whitespace-nowrap">
                    <thead className="sticky top-0 bg-[#f4f7fe] dark:bg-[#151521] z-10">
                      ...
                      {adminUsers.map(user => (
                        <tr key={user.id}>
                          ...
                          <td className="py-4 px-2">
                            <span className={`px-2 py-1 rounded text-xs font-bold capitalize ${(!user.role || user.role.toLowerCase() === 'admin') ? 'bg-blue-100 text-blue-600 dark:bg-blue-900/30 dark:text-blue-400' : 'bg-gray-800 text-gray-300'}`}>
                              {user.role || 'Admin'}
                            </span>
                          </td>
          ...
          {activeTab === 'companies' && (
            <div className="p-4 md:p-8 w-full mx-auto space-y-6 animate-[fadeIn_0.3s_ease-out]">
              <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
                <div>
                  <h2 className="text-3xl font-bold font-heading mb-1">Companies</h2>
                  <p className="text-gray-500 dark:text-gray-400 text-sm">Manage registered companies and their trafo lists.</p>
                </div>
              </div>

              {showCompanyModal && (
                <div className="bg-[#f4f7fe] dark:bg-[#151521] border border-gray-200 dark:border-white/10 rounded-2xl p-6">
                  <h3 className="font-bold text-lg mb-4">Edit Company</h3>
                  <form onSubmit={handleSaveCompany} className="space-y-4">
                    {companyFormError && <p className="text-red-400 text-sm">{companyFormError}</p>}
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      <div>
                    ...
                    <div className="flex justify-end gap-3 pt-4">
                      <button type="button" onClick={() => setShowCompanyModal(false)} className="px-4 py-2 rounded-lg font-semibold hover:bg-gray-100 dark:hover:bg-gray-800 transition-colors">Cancel</button>
                      <button type="submit" disabled={isSavingCompany} className="px-4 py-2 bg-blue-600 hover:bg-blue-700 rounded-lg font-bold transition-colors">Save</button>
                    </div>
                  </form>
                </div>
              )}

              <div className="bg-[#f4f7fe] dark:bg-[#151521] border border-gray-200 dark:border-white/10 rounded-2xl overflow-hidden flex flex-col max-h-[70vh]">
                <div className="p-4 border-b border-gray-200 dark:border-white/5 relative">
                  ...
                  <input
                    ...
                    className="w-full md:w-64 bg-gray-100 dark:bg-black/20 border border-gray-200 dark:border-white/10 rounded-lg pl-10 pr-4 py-2 text-sm outline-none focus:border-white/30 text-[#172b4d] dark:text-white transition-colors"
                  />
                </div>
                <div className="flex-1 overflow-auto custom-scrollbar">
                  <table className="w-full text-left text-sm whitespace-nowrap relative">
          ...
          {activeTab === 'users' && (
            <div className="p-4 md:p-8 w-full mx-auto space-y-6 animate-[fadeIn_0.3s_ease-out]">
              ...
              {showAppUserModal && (
                <div className="bg-[#f4f7fe] dark:bg-[#151521] border border-gray-200 dark:border-white/10 rounded-2xl p-6">
                  <h3 className="font-bold text-lg mb-4">{editingAppUser ? 'Edit User' : 'Add User'}</h3>
              ...
              <div className="bg-[#f4f7fe] dark:bg-[#151521] border border-gray-200 dark:border-white/10 rounded-2xl p-4 md:p-6 flex flex-col h-[70vh]">
                ...
                <div className="flex-1 overflow-auto custom-scrollbar">
                    ...
                    <tbody className="divide-y divide-white/5">
                      {allUsers.filter(u => {
                        ...
                        <tr key={u.id} className="hover:bg-gray-100 dark:hover:bg-gray-800">
                          ...
                          <td className="p-4 text-gray-500 dark:text-gray-300">{u.nama_db || '-'}</td>
                          <td className="p-4 text-gray-500 dark:text-gray-300">{u.nomor_telpon || '-'}</td>
                          <td className="p-4 text-gray-500 dark:text-gray-300">{u.email || '-'}</td>
                          <td className="p-4">
                            <span className={`px-2 py-1 rounded text-xs font-bold capitalize ${u.role === 'admin' ? 'bg-blue-100 text-blue-600 dark:bg-blue-900/30 dark:text-blue-400' : u.role === 'superuser' ? 'bg-green-100 text-green-600 dark:bg-green-900/30 dark:text-green-400' : 'bg-yellow-100 text-yellow-600 dark:bg-yellow-900/30 dark:text-yellow-400'}`}>
                              {u.role}
                            </span>
                          </td>
                  ...
                  <div className="md:hidden space-y-4 pb-16">
                    {allUsers.filter(u => {
                      ...
                      <div key={u.id} className="bg-[#f4f7fe] dark:bg-[#1a1a24] border border-gray-200 dark:border-white/5 rounded-2xl p-5 shadow-sm">
                        <div className="flex justify-between items-center mb-4 pb-4 border-b border-gray-200 dark:border-white/5">
                          <span className="font-bold text-lg">{u.username}</span>
                          <div className="flex items-center gap-2">
                            <span className={`px-3 py-1 rounded-full text-xs font-bold capitalize ${u.role === 'admin' ? 'bg-blue-100 text-blue-600 border border-blue-200 dark:bg-blue-900/30 dark:text-blue-400 dark:border-blue-500/30' : u.role === 'superuser' ? 'bg-green-100 text-green-600 border border-green-200 dark:bg-green-900/30 dark:text-green-400 dark:border-green-500/30' : 'bg-yellow-100 text-yellow-600 border border-yellow-200 dark:bg-yellow-900/30 dark:text-yellow-400 dark:border-yellow-500/30'}`}>
                              {u.role}
                            </span>
            ...
            <div className="p-4 md:p-8 max-w-4xl mx-auto animate-[fadeIn_0.3s_ease-out] flex flex-col items-center justify-center min-h-[70vh] md:min-h-[80vh]">
              {!waStatus.ready ? (
                <div className="text-center w-full max-w-md">
                  ...
                  <div className="bg-[#f4f7fe] dark:bg-[#151521] border border-gray-200 dark:border-white/10 p-6 md:p-8 rounded-3xl flex items-center justify-center shadow-2xl mb-8 mx-auto w-64 h-64 md:w-80 md:h-80 relative overflow-hidden">
                    {isReloadingWA ? (
                ...
                <div className="text-center w-full max-w-lg">
                  ...
                  <div className="bg-[#f4f7fe] dark:bg-[#151521] border border-gray-200 dark:border-white/10 p-6 md:p-8 rounded-3xl text-left shadow-2xl">
                    <div className="flex items-center gap-4 md:gap-5 mb-8">
                      <div className="w-14 h-14 md:w-16 md:h-16 bg-white border border-green-500 dark:bg-green-900/30 dark:border-transparent rounded-full flex items-center justify-center shrink-0">
                    ...
                    <button
                      ...
                      className="w-full py-3 md:py-4 border bg-white border-red-500 text-red-600 hover:bg-red-50 dark:bg-transparent dark:border-red-900/50 dark:text-red-400 dark:hover:bg-red-900/20 rounded-xl font-bold flex items-center justify-center gap-2 transition-colors disabled:opacity-50"
                    >
      ...
      <nav className="md:hidden fixed bottom-0 left-0 right-0 bg-white dark:bg-[#151521] border-t border-gray-200 dark:border-white/5 flex items-center overflow-x-auto overflow-y-hidden flex-nowrap scrollbar-hide p-2 gap-2 z-50 pb-safe">
        ...
        <BottomNavItem icon={Building2} label="Companies" id="companies" activeTab={activeTab} setActiveTab={setActiveTab} />
