...
router.post('/generate-token', authenticateToken, async (req, res) => {
  ...
  const { company_code, company_name, db_name, tmu_username, tmu_password, transformers_list, tmu_version } = req.body;
  
  if (!company_code || !company_name || !db_name || !tmu_username || !tmu_password || !transformers_list) {
    return res.status(400).json({ error: 'Semua field (company_code, company_name, db_name, username, password, transformers) wajib diisi' });
  }
  ...
  try {
    ...
    while (!isUnique) {
      ...
      const existingToken = await prisma.provisionToken.findUnique({ where: { token } });
...
router.put('/tokens/:id', authenticateToken, async (req, res) => {
  ...
  try {
    const token = await prisma.provisionToken.findUnique({ where: { id: parseInt(id) } });
    if (!token) return res.status(404).json({ error: 'Token not found' });
    if (token.is_used) return res.status(400).json({ error: 'Cannot edit an already used token' });
...
router.get('/generate-company-code', authenticateToken, async (req, res) => {
  ...
  try {
    ...
    while (!isUnique) {
      ...
      const existingInTokens = await prisma.provisionToken.findFirst({ where: { company_code: finalCode } });
      const existingInDevices = await prisma.registeredDevice.findFirst({ where: { company_code: finalCode } });
...
router.post('/activate', async (req, res) => {
  ...
  if (!provision_token || !serial_number) {
    return res.status(400).json({ status: 400, message: 'Missing token or serial_number' });
  }
  
  try {
    const tokenRow = await prisma.provisionToken.findUnique({ where: { token: provision_token } });
    
    if (!tokenRow || tokenRow.is_used === 1) {
      return res.status(400).json({ status: 400, message: 'Token tidak valid atau sudah dipakai' });
    }
    
    if (new Date() > new Date(tokenRow.expires_at)) {
      return res.status(400).json({ status: 400, message: 'Token sudah expired' });
    }
    ...
    const programKey = CryptoJS.AES.encrypt(hashedSerial, SECRET_KEY).toString();
    ...
    if (!clientUsername || !clientPassword) {
      return res.status(400).json({ status: 400, message: 'Token tidak lengkap (dibuat di versi lama). Harap generate ulang token baru.' });
    }
    ...
    return res.json({
      ...
      env_config: {
        ...
        DB_USER: 'client',
        DB_PASSWORD: 'raspi',
        DB_NAME: 'iot_trafo_client',
        COMPANY_ID: tokenRow.company_code,
        COMPANY_NAME: tokenRow.company_name,
        COMPANY_DATABASE: tokenRow.db_name,
        COMPANY_KEY: companyData.company_key,
        PROGRAM_KEY: programKey,
        TELEGRAM_TOKEN: companyData.telegram_api_token,
        CHAT_ID: companyData.telegram_chat_id,
        SERVER_DOMAIN: 'tmu.bambangdjaja.com',
        TMU_USERNAME: clientUsername,
        TMU_PASSWORD: clientPassword,
        TRANSFORMERS_LIST: tokenRow.transformers_list
      },
      tmu_version: tokenRow.tmu_version,
